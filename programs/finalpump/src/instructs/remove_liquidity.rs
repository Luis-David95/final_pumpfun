...
pub fn remove_liquidity(ctx: Context<RemoveLiquidity>, bump: u8) -> Result<()> {
    ...
    if pool.creator.key() != ctx.accounts.user.key() {return Err(CustomError::NotCreator.into());}
    ...
    let token_accounts = (&mut *ctx.accounts.token_mint,&mut *ctx.accounts.pool_token_account,&mut *ctx.accounts.user_token_account);

    pool.remove_liquidity(token_accounts,&mut ctx.accounts.pool_sol_vault,&ctx.accounts.user,bump,&ctx.accounts.token_program,&ctx.accounts.system_program)?;
    Ok(())
}
...
pub struct RemoveLiquidity<'info> {
    ...
    #[account(mut,associated_token::mint = token_mint,associated_token::authority = pool)]
    ...
    /// CHECK:
    #[account(mut,seeds = [LiquidityPool::SOL_VAULT_PREFIX.as_bytes(), token_mint.key().as_ref()],bump)]
